...
Writing a 256K overlay and passing a catalog doctor does not prove that an already-running Router loaded it. A compatibility adapter can fall back safely, but the real 256K route is proved only by a completed request receipt. Reload only in a maintenance window: restarting a loopback service can sever an in-flight stream.
